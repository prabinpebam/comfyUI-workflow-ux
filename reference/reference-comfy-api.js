...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Helper: Convert a blob to a JPEG data URL with highest quality.
  ...
  async function loadUserEditableParameters() {
    try {
      ...
      for (const nodeId in paramsJson) {
        ...
        // Create a container for this node's fields (node name is omitted)
  ...
  async function handleEditableFile(file, nodeId, inputKey, fieldId) {
    ...
    try {
      ...
      const jpegDataUrl = await convertBlobToJpeg(file);
  ...
  // Helper: Convert blob to JPEG data URL (already defined above as convertBlobToJpeg)
  // (The old blobToBase64 function is removed.)
  ...
  function displayImages(images, container) {
    images.forEach(async (imageInfo) => {
      ...
      try {
        ...
        const base64Data = await convertBlobToJpeg(blob);
